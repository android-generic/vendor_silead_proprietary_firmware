...
def list_whence():
    with open('WHENCE.silead', encoding='utf-8') as whence:
        for line in whence:
...
def main():
    ...
    known_files = set(name for name in whence_list if not name.endswith('/')) | \
                  set(['check_whence.py', 'README.md', 'copy-firmware.sh', 'WHENCE.silead'])
    known_prefixes = set(name for name in whence_list if name.endswith('/'))
    git_files = set(list_git())

    for name in set(name for name in whence_files if name.endswith('/')):
        sys.stderr.write('E: %s listed in WHENCE.silead as File, but is directory\n' %
                         name)
        ret = 1

    for name in set(fw for fw in whence_files if whence_files.count(fw) > 1):
        sys.stderr.write('E: %s listed in WHENCE.silead twice\n' % name)
        ret = 1

    for name in set(link for link in whence_files if os.path.islink(link)):
        sys.stderr.write('E: %s listed in WHENCE.silead as File, but is a symlink\n' %
                         name)
        ret = 1

    for name in set(link[0] for link in links_list if os.path.islink(link[0])):
        sys.stderr.write('E: %s listed in WHENCE.silead as Link, is in tree\n' % name)
        ret = 1

    for name in sorted(list(known_files - git_files)):
        sys.stderr.write('E: %s listed in WHENCE.silead does not exist\n' % name)
        ret = 1
    ...
    for name, target in sorted(links_list):
        if target not in valid_targets:
            sys.stderr.write('E: target %s of link %s in WHENCE.silead'
                             ' does not exist\n' % (target, name))
            ret = 1
    ...
    for name in sorted(list(git_files - known_files)):
        ...
        for prefix in known_prefixes:
            ...
            sys.stderr.write('E: %s not listed in WHENCE.silead\n' % name)
            ret = 1
    return ret
