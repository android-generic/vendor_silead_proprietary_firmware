...
[ -z "$FIRMWARE_DIR" ] && FIRMWARE_DIR=vendor/silead/proprietary/firmware/firmware/linux
...
grep '^File:' WHENCE.silead | sed -e 's/^File: *//g;s/"//g' | while read f; do
    test -f "$f" || continue
    install -d "$destdir/$(dirname "$f")"
    $verbose "copying/compressing file $f$compext"
    if test "$compress" != "cat" && grep -q "^Raw: $f\$" WHENCE.silead; then
...
# shellcheck disable=SC2162 # file/folder name can include escaped symbols
grep -E '^Link:' WHENCE.silead | sed -e 's/^Link: *//g;s/-> //g' | while read f d; do
